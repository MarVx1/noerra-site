...
# Аббревиатуры (ADNI, AIBL, OASIS-3, ДНК-подобные названия датасетов/генов) —
# сплошные заглавные буквы, не переводятся ни при каких обстоятельствах.
# Раньше это блокировало публикацию целиком (см. историю теста
# test_latin_text_blocks_publication) — решение пересмотрено 2026-07-15:
# полное блокирование одинаково задевает и уместные акронимы (названия
# когорт/датасетов вроде ADNI), и названия генов (ADGRL3) — по форме их не
# отличить, а блокировать нормальные статьи из-за названия датасета вреднее,
# чем изредка пропустить ген в качестве подлежащего.
...
class EditorialCritic:
    ...
    def check_language_is_russian(self, text: str) -> List[str]:
        ...
        кроме сплошных заглавных аббревиатур (ADNI, AIBL, OASIS-3, ADGRL3 —
        см. _is_acronym): их не переводят даже в качественных переводах, и
        rule-based невозможно отличить уместный акроним датасета от
        названия гена по одной только форме слова, см. комментарий у
        _ACRONYM_RE.
        ...
        без латиницы при разборе абстракта). Но остаётся утечка настоящего
        непереведённого английского текста (не акронимов) — она блокирует
        публикацию: лучше не опубликовать ничего, чем выпустить наполовину
        английский текст.
        """
        ...
        found = {
            ...
            if w.lower() not in ALLOWED_LATIN_WORDS and not _is_acronym(w)
        }
        if found:
