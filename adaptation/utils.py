...
def _capitalize_sentences(text: str) -> str:
    ...
    [^\\w<]* вместо \\W* — исключаем "<" из символов, которые можно
    пропустить перед первой буквой: build_structure() оборачивает аналогию
    в <i>...</i>, а доказательность/ограничения в <b>...</b>, и эта функция
    вызывается (через simplify_text() в generate_text()) на КАЖДОМ блоке
    структуры, включая эти — блок вида "<i>Привычки поведения похожи..."
    старым \\W*-паттерном читался как "пропустить '<', заглавить следующую
    букву", а следующая буква — это 'i' самого тега, что превращало его в
    "<I>" (реальный опубликованный пример, article id=1064, 2026-07-20:
    открывающий тег заглавный, закрывающий — нет, т.к. до него нет
    "[.!?]\\s+"). С [^\\w<]* тег в начале блока просто не матчится (в нём
    и так уже нет строчных первых букв для восстановления — сами шаблоны
    аналогий/лейблов пишутся с заглавной буквы после тега)."""
    if not text:
        return text
