...
Запуск:
    python scripts/generate_site.py            # docs/ + published.json + git push
    python scripts/generate_site.py --no-git   # без git-шага (локальный предпросмотр)
"""
...
# scripts/ — чтобы найти соседний site_builder.py; корень проекта — чтобы
# найти adaptation/classifier/config/database (ТЗ 2026-07-20, п.2: раньше
# без ручного PYTHONPATH=. падало с ModuleNotFoundError на "adaptation").
...
from classifier.classifier import get_topic_ru, get_topic_emoji, get_evidence_label, get_evidence_emoji  # noqa: E402
...
def _evidence_badge(evidence_strength: str | None) -> str | None:
    """Ярлык и эмодзи — из classifier.classifier (единый источник истины
    для evidence_strength, ТЗ 2026-07-20), не свой словарь: то же слово,
    что показывает бот в карточке модерации и что теперь пишет сам
    generate_text() внутри тела поста (adaptation/editorial_engine.py)."""
    if not evidence_strength:
        return None
    emoji = get_evidence_emoji(evidence_strength)
    if not emoji:
        return None
    return f"{emoji} {get_evidence_label(evidence_strength)}"
