"""Тесты Telegram-слоя (bot/bot.py).
...
class TestGetDigestCandidatesInfo(unittest.TestCase):
    def test_groups_by_topic_and_counts(self):
        ...
        with patch.object(b, "get_top_articles_by_topic", return_value=rows):
            total, topics = b.get_digest_candidates_info()
    ...
    def test_missing_topic_falls_back_to_unknown(self):
        with patch.object(b, "get_top_articles_by_topic", return_value=[{"topic": None, "title": "X"}]):
            _, topics = b.get_digest_candidates_info()
        self.assertIn("unknown", topics)

    def test_count_helper_returns_total(self):
        rows = [{"topic": "sleep", "title": "A"}, {"topic": "sleep", "title": "B"}]
        with patch.object(b, "get_top_articles_by_topic", return_value=rows):
            self.assertEqual(b.get_digest_candidates_count(), 2)
...
class TestPublishArticle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_error_when_article_missing(self):
        with patch.object(b, "get_article_by_id", return_value=None):
            ok, msg = await b._publish_article(1)
        self.assertFalse(ok)
        self.assertEqual(msg, "Статья не найдена")

    async def test_returns_error_when_telegraph_fails(self):
        with patch.object(b, "get_article_by_id", return_value=self.ARTICLE), \
             patch.object(b, "create_telegraph_page", AsyncMock(return_value=None)):
            ok, msg = await b._publish_article(1)
        self.assertFalse(ok)
    ...
    async def test_success_substitutes_telegraph_url_and_saves(self):
        ...
        with patch.object(b, "get_article_by_id", return_value=self.ARTICLE), \
             patch.object(b, "create_telegraph_page", AsyncMock(return_value=url)), \
             patch.object(b, "send_to_channel", send), \
             patch.object(b, "update_article_status") as upd, \
             patch.object(b, "save_publication") as save:
            ok, result = await b._publish_article(1)
    ...
    async def test_builds_fallback_post_text_when_empty(self):
        ...
        with patch.object(b, "get_article_by_id", return_value=article), \
             patch.object(b, "create_telegraph_page", AsyncMock(return_value="https://telegra.ph/y")), \
             patch.object(b, "send_to_channel", send), \
             patch.object(b, "update_article_status"), \
             patch.object(b, "save_publication"):
            ok, _ = await b._publish_article(1)
    ...
    async def test_saves_zero_message_id_when_channel_send_returns_none(self):
        with patch.object(b, "get_article_by_id", return_value=self.ARTICLE), \
             patch.object(b, "create_telegraph_page", AsyncMock(return_value="https://telegra.ph/z")), \
             patch.object(b, "send_to_channel", AsyncMock(return_value=None)), \
             patch.object(b, "update_article_status"), \
             patch.object(b, "save_publication") as save:
            await b._publish_article(1)
...
class TestSendForModeration(unittest.IsolatedAsyncioTestCase):
    ...
    def _mock_bot(send_message):
        ...
        return patch.object(b, "get_bot", return_value=fake)

    async def test_does_nothing_when_article_missing(self):
        send = AsyncMock()
        with patch.object(b, "get_article_by_id", return_value=None), \
             self._mock_bot(send):
            await b.send_for_moderation(1)
        send.assert_not_awaited()

    async def test_sends_message_and_marks_pending(self):
        send = AsyncMock()
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "get_article_by_id", return_value=self.ARTICLE), \
             self._mock_bot(send), \
             patch.object(b, "update_article_status") as upd:
            await b.send_for_moderation(1)
...
class TestApproveRejectCallbacks(unittest.IsolatedAsyncioTestCase):
    async def test_approve_denied_for_non_admin(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "_publish_article", AsyncMock()) as pub:
            await b.on_approve(cb)
    ...
    async def test_approve_publishes_and_appends_result(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "_publish_article", AsyncMock(return_value=(True, "https://telegra.ph/a"))) as pub:
            await b.on_approve(cb)

        pub.assert_awaited_once_with(42)
        self.assertIn("Опубликовано", cb.message.edit_text.await_args.args[0])

    async def test_approve_appends_error_on_failure(self):
        cb = _callback("approve:42")
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "_publish_article", AsyncMock(return_value=(False, "boom"))):
            await b.on_approve(cb)
...
class TestDraftCallbacks(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_draft_approve_denied_for_non_admin(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "save_editor_feedback") as fb:
            await b.on_draft_approve(cb)
        fb.assert_not_called()

    async def test_draft_approve_reports_missing_draft(self):
        cb = _callback("draft_approve:5")
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "get_draft_by_id", return_value=None), \
             patch.object(b, "save_editor_feedback") as fb:
            await b.on_draft_approve(cb)
    ...
    async def test_draft_approve_saves_feedback_and_shows_publish_button(self):
        cb = _callback("draft_approve:5")
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "get_draft_by_id", return_value=self.DRAFT), \
             patch.object(b, "save_editor_feedback") as fb, \
             patch.object(b, "update_article_status") as upd:
            await b.on_draft_approve(cb)
        ...
        self.assertEqual(kb.inline_keyboard[0][0].callback_data, "draft_publish:5")
    ...
    async def test_draft_publish_requires_article_id(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "get_draft_by_id", return_value={"article_id": None}), \
             patch.object(b, "_publish_article", AsyncMock()) as pub:
            await b.on_draft_publish(cb)
    ...
    async def test_draft_publish_publishes_linked_article(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "get_draft_by_id", return_value=self.DRAFT), \
             patch.object(b, "_publish_article", AsyncMock(return_value=(True, "https://telegra.ph/q"))) as pub:
            await b.on_draft_publish(cb)

        pub.assert_awaited_once_with(10)
...
class TestDigestCallbacks(unittest.IsolatedAsyncioTestCase):
    async def test_digest_approve_publishes_all_ids_and_counts_successes(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "_publish_article", AsyncMock(side_effect=results)) as pub:
            await b.on_digest_approve(cb)

        self.assertEqual([c.args[0] for c in pub.await_args_list], [1, 2, 3])
    ...
    async def test_digest_one_by_one_sends_each_for_moderation(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "send_for_moderation", AsyncMock()) as send:
            await b.on_digest_one_by_one(cb)
    ...
    async def test_digest_reject_all_marks_skipped(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN):
            await b.on_digest_reject_all(cb)
...
class TestInvalidateCacheCommand(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_shows_usage_without_arguments(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "invalidate_translations_matching") as inv:
            await b.cmd_invalidate_cache(msg)
    ...
    async def test_reports_when_nothing_matches(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "count_translations_matching", return_value=0), \
             patch.object(b, "invalidate_translations_matching") as inv:
            await b.cmd_invalidate_cache(msg)
    ...
    async def test_preview_does_not_delete_without_confirm(self):
        msg = _message("/invalidate_cache наградум")
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "count_translations_matching", return_value=3), \
             patch.object(b, "invalidate_translations_matching") as inv:
            await b.cmd_invalidate_cache(msg)
    ...
    async def test_confirm_deletes_matching_rows(self):
        ...
        with patch.object(b, "ADMIN_ID", ADMIN), \
             patch.object(b, "invalidate_translations_matching", return_value=2) as inv:
            await b.cmd_invalidate_cache(msg)
