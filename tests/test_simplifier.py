...
class TestSimplifyText(unittest.TestCase):
    ...
    def test_removes_v_dannoy_rabote(self):
        ...
        # Оборот стоял в начале предложения, поэтому после его удаления
        # заглавная восстанавливается — иначе абзац начинался бы со строчной.
        self.assertEqual(result, "Рассматривается влияние сна на память.")

    def test_recapitalizes_after_removing_leading_phrase(self):
        """Регрессия: 'Таким образом, пищевое вознаграждение...' превращалось
        в 'пищевое вознаграждение...' — предложение со строчной буквы."""
        text = "Тренировки помогают. Таким образом, пищевое вознаграждение является основой."
        self.assertEqual(
            simplify_text(text),
            "Тренировки помогают. Пищевое вознаграждение является основой.",
        )
